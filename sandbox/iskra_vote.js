...
net.connect({host: '192.168.88.182', port: 1337}, function(socket) {
  ...
  socket.on('data', function(recieved) {
    ...
    var dataDoors = data.doorsitem;
    var ptintOut = '', ptintOutAngle = '';      // DELETE!!!
    // для каждой двери обновляем текущее состояние
    for (var door in dataDoors) {
      doors[door].state = dataDoors[door].st;
